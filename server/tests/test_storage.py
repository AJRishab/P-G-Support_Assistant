...
@pytest.mark.asyncio
async def test_session_persistence_across_restart(clean_db):
    session_id = "test_session_123"
    
    # Write messages to database
    await clean_db.save_message(session_id, "user", "Hello support!")
    await clean_db.save_message(session_id, "assistant", "Hello! How can I help you?")
    
    # Retrieve and verify
    history1 = await clean_db.get_history(session_id)
    ...
    history2 = await new_db_instance.get_history(session_id)
    assert len(history2) == 2
    assert history2[0]["content"] == "Hello support!"
    assert history2[1]["content"] == "Hello! How can I help you?"

@pytest.mark.asyncio
async def test_ticket_creation_and_retrieval(clean_db):
    session_id = "test_safety_session"
    
    await clean_db.create_ticket(
    ...
    tickets = await clean_db.get_tickets()
